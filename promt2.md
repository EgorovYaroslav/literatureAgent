# Project Update: Structured Literature Review Workflow & Bug Fix

## Context
We are refining the literature review automation project. We do not need to run experiments right now. Instead, we need to establish a minimal, highly structured project environment, fix a known bug in our tooling, and rewrite `AGENT.md` to enforce a strict, step-by-step workflow. We will use `git` for version control, and the new `AGENT.md` must strictly follow the format of an existing `AGENT_example.md` file.
...
## Step 2: Debug and Fix `tools/scihub_tool.py`
**CRITICAL**: There is a known issue with downloading articles in `tools/scihub_tool.py`.
1. Read and inspect `tools/scihub_tool.py` (and the underlying Sci-Hub MCP server logic if necessary).
2. Identify why the PDF download functionality is failing.
3. Fix the code to ensure it reliably downloads PDFs and saves them to the specified directory.
4. Commit this fix to git: `git add tools/scihub_tool.py && git commit -m "fix: resolve article download issue in scihub_tool"`.

## Step 3: Create Tracking and Documentation Files
...
## Step 4: Rewrite `AGENT.md` using `AGENT_example.md` as a Template
...
- Ensure `git` is used to track the creation of files and the bug fix.
- **Do not invent a new format for `AGENT.md`**; you must derive the step-by-step structure directly from `AGENT_example.md`.
- Double-check that `tools/scihub_tool.py` is actually debugged and the download logic is corrected before finalizing the setup.